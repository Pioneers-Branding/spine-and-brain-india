<!DOCTYPE html>
<html lang="en">

<head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
        <meta name="description" content="Expert head injury treatment by Dr. Arun Saroha. Advanced emergency trauma care and brain surgery. 26+ years experience. Get expert neurological care. Consult now!" />
        <meta name="keywords" content="best head injury treatment in india, head injury treatment, neurosurgeon for head injury, brain trauma surgery india, head injury surgery cost, traumatic brain injury treatment, head injury specialist, head injury rehabilitation india, TBI treatment india, best neurologist for head injury" />

        <title>Best Head Injury Treatment in India | Dr. Arun Saroha</title>

        <link rel="canonical" href="https://spineandbrainindia.com/head-injury" />

<!-- Unified SEO Schema and Tags -->
<meta name="geo.region" content="IN" />
<meta name="geo.placename" content="India" />
<meta name="geo.position" content="28.4595;77.0266" />
<meta name="ICBM" content="28.4595, 77.0266" />
<meta name="robots" content="index, follow" />

<meta property="og:type" content="website" />
<meta property="og:url" content="https://spineandbrainindia.com/head-injury" />
<meta property="og:title" content="Best Head Injury Treatment in India | Dr. Arun Saroha" />
<meta property="og:description" content="Expert head injury treatment by Dr. Arun Saroha with 26+ years experience. Advanced neurosurgical care, emergency trauma services. Call +91-7860000705." />
<meta property="og:image" content="https://spineandbrainindia.com/assets/images/pag-top-bg_11zon.webp" />
<meta property="og:site_name" content="Spine and Brain India" />

<meta name="twitter:card" content="summary_large_image" />
<meta name="twitter:title" content="Best Head Injury Treatment in India | Dr. Arun Saroha" />
<meta name="twitter:description" content="Expert head injury treatment by Dr. Arun Saroha. Advanced neurosurgical care at Max Hospital, Delhi NCR." />

<script type="application/ld+json">
{
  "@context": "https://schema.org",
  "@graph": [
    {
      "@type": "Physician",
      "@id": "https://spineandbrainindia.com/dr-arun-saroha.php",
      "name": "Dr. Arun Saroha",
      "url": "https://spineandbrainindia.com/dr-arun-saroha.php",
      "image": "https://spineandbrainindia.com/assets/images/resources2/drarundef_11zon.webp",
      "description": "Dr. Arun Saroha is a leading neurosurgeon in India with over 26+ years of experience specializing in head injury treatment, traumatic brain injury surgery, and neurotrauma care.",
      "medicalSpecialty": "Neurosurgery",
      "knowsAbout": ["Head Injury Treatment", "Traumatic Brain Injury", "Craniotomy", "Neurotrauma Surgery", "Brain Hemorrhage Treatment"],
      "aggregateRating": {
        "@type": "AggregateRating",
        "ratingValue": "4.9",
        "bestRating": "5",
        "reviewCount": "1045"
      },
      "address": {
        "@type": "PostalAddress",
        "streetAddress": "Max Super Speciality Hospital, Sector 19",
        "addressLocality": "Saket, New Delhi",
        "addressRegion": "Delhi",
        "postalCode": "110017",
        "addressCountry": "IN"
      }
    },
    {
      "@type": "MedicalClinic",
      "name": "Spine and Brain India",
      "url": "https://spineandbrainindia.com/head-injury",
      "image": "https://spineandbrainindia.com/assets/img/logo/logoSSB%207777777_11zon.webp",
      "telephone": "+91-7860000705",
      "address": {
        "@type": "PostalAddress",
        "streetAddress": "Max Super Speciality Hospital",
        "addressLocality": "Gurgaon",
        "addressRegion": "Haryana",
        "addressCountry": "IN"
      },
      "aggregateRating": {
        "@type": "AggregateRating",
        "ratingValue": "4.8",
        "bestRating": "5",
        "reviewCount": "850"
      },
      "geo": {
        "@type": "GeoCoordinates",
        "latitude": "28.4595",
        "longitude": "77.0266"
      },
      "medicalSpecialty": "Neurosurgery",
      "availableService": {
        "@type": "MedicalProcedure",
        "name": "Head Injury Treatment",
        "procedureType": "Surgical"
      }
    },
    {
      "@type": "MedicalCondition",
      "name": "Head Injury",
      "alternateName": ["Traumatic Brain Injury", "TBI", "Brain Trauma", "Craniocerebral Trauma"],
      "associatedAnatomy": {
        "@type": "AnatomicalStructure",
        "name": "Brain"
      },
      "possibleTreatment": [
        {
          "@type": "MedicalProcedure",
          "name": "Craniotomy for Head Injury"
        },
        {
          "@type": "MedicalProcedure",
          "name": "Decompressive Craniectomy"
        },
        {
          "@type": "MedicalProcedure",
          "name": "Hematoma Evacuation"
        }
      ]
    },
    {
      "@type": "FAQPage",
      "mainEntity": [
        {
          "@type": "Question",
          "name": "What is a head injury and how serious can it be?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "A head injury, also known as traumatic brain injury (TBI), is any injury to the brain, skull, or scalp caused by external force such as a blow, jolt, or penetrating wound. Head injuries range from mild concussions that resolve in days to severe brain damage that can be life-threatening or cause permanent disability. Early diagnosis and treatment by an experienced neurosurgeon significantly improves outcomes."
          }
        },
        {
          "@type": "Question",
          "name": "What are the warning signs of a serious head injury?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "Warning signs of a serious head injury include: prolonged loss of consciousness, repeated vomiting, seizures, unequal pupil size, clear fluid from nose or ears, severe headache that worsens, weakness or numbness in limbs, slurred speech, increasing confusion, and difficulty waking from sleep. If any of these symptoms appear after a head trauma, seek emergency neurosurgical care immediately by calling +91-7860000705."
          }
        },
        {
          "@type": "Question",
          "name": "What causes head injuries?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "The most common causes of head injuries in India include road traffic accidents (the leading cause at nearly 60%), falls (especially in elderly and children), sports-related injuries, workplace accidents, physical assault, and falling objects. Two-wheeler accidents without helmets are a particularly significant cause of severe TBI in India."
          }
        },
        {
          "@type": "Question",
          "name": "How is a head injury diagnosed?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "Head injury diagnosis involves a thorough neurological examination using the Glasgow Coma Scale (GCS), followed by CT scan (the primary imaging tool for emergency cases) and MRI for detailed brain tissue assessment. Additional tests may include X-rays for skull fractures, angiography for blood vessel damage, and intracranial pressure (ICP) monitoring for severe cases."
          }
        },
        {
          "@type": "Question",
          "name": "What are the treatment options for head injuries?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "Treatment depends on severity. Mild injuries (concussions) are managed with rest, observation, and pain medication. Moderate to severe injuries may require surgical intervention including craniotomy (opening the skull to remove blood clots), decompressive craniectomy (removing skull bone to relieve pressure), hematoma evacuation, depressed fracture repair, and ICP monitoring. Dr. Arun Saroha determines the best approach based on each patient's specific condition."
          }
        },
        {
          "@type": "Question",
          "name": "How much does head injury treatment cost in India?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "Head injury treatment costs in India vary based on severity. Mild injury observation costs INR 50,000-1,50,000. Surgical procedures like craniotomy or hematoma evacuation range from INR 2,00,000-5,00,000. Decompressive craniectomy costs INR 3,00,000-6,00,000. ICU stays cost INR 15,000-35,000 per day. These costs are significantly lower than Western countries while maintaining world-class care standards. Contact us at +91-7860000705 for a personalized estimate."
          }
        },
        {
          "@type": "Question",
          "name": "What is the recovery time after head injury surgery?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "Recovery time varies significantly based on injury severity. Mild concussions typically resolve in 2-4 weeks. Moderate head injuries require 3-6 months for recovery. Severe TBI cases may need 6-18 months or more of rehabilitation. Factors affecting recovery include patient age, overall health, extent of brain damage, and quality of post-surgical rehabilitation."
          }
        },
        {
          "@type": "Question",
          "name": "What is the Golden Hour in head injury treatment?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "The Golden Hour refers to the first 60 minutes after a severe head injury. Getting the patient to a qualified neurosurgeon within this critical window dramatically improves survival rates and recovery outcomes. During this time, life-saving interventions like stopping brain bleeding, reducing intracranial pressure, and preventing secondary brain damage can be performed. Dr. Arun Saroha's team at Max Hospital is available 24/7 for emergency head injury cases."
          }
        },
        {
          "@type": "Question",
          "name": "Can a head injury cause long-term complications?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "Yes, depending on severity. Possible long-term complications include post-traumatic epilepsy (seizures), cognitive difficulties (memory, concentration), behavioral and personality changes, chronic headaches, balance problems, speech difficulties, and in severe cases, physical disability. However, with expert treatment from a skilled neurosurgeon like Dr. Saroha, followed by proper rehabilitation, many patients achieve significant recovery and return to their daily activities."
          }
        },
        {
          "@type": "Question",
          "name": "Is head injury treatment available for international patients?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "Yes, Dr. Arun Saroha and Spine and Brain India regularly treat international patients from the Middle East, Africa, and Southeast Asia. We provide complete assistance including medical visa support, airport pickup, accommodation arrangements, interpreter services, and telemedicine follow-up consultations. India offers the best head injury treatment at 60-80% lower costs compared to the US, UK, or Europe."
          }
        },
        {
          "@type": "Question",
          "name": "What rehabilitation is needed after head injury surgery?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "Post-surgical rehabilitation is crucial for optimal recovery and may include: physiotherapy (for motor function and strength), occupational therapy (for daily living skills), speech and language therapy, cognitive rehabilitation (for memory and thinking skills), neuropsychological counseling, and nutritional support. Dr. Saroha's team creates personalized rehabilitation plans for each patient based on their specific deficits and recovery goals."
          }
        },
        {
          "@type": "Question",
          "name": "How can head injuries be prevented?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "Key prevention measures include: always wearing helmets while riding two-wheelers or cycling, using seatbelts in vehicles, installing safety gates for children, using non-slip mats for elderly, following workplace safety protocols, avoiding driving under the influence of alcohol, supervising children during sports, and wearing protective gear during contact sports. While not all head injuries can be prevented, these precautions significantly reduce the risk."
          }
        }
      ]
    },
    {
      "@type": "BreadcrumbList",
      "itemListElement": [
        {
          "@type": "ListItem",
          "position": 1,
          "name": "Home",
          "item": "https://spineandbrainindia.com/"
        },
        {
          "@type": "ListItem",
          "position": 2,
          "name": "Head Injury Treatment",
          "item": "https://spineandbrainindia.com/head-injury"
        }
      ]
    }
  ]
}
</script>
<!-- End Unified SEO Schema and Tags -->

<?php include 'header.php';?>
        <section>
                <div class="w-100 pt-100 black-layer opc5 pb-80 position-relative">
                    <div class="fixed-bg" style="background-image: url(assets/images/pag-top-bg.jpg);"></div>
                    <div class="container">
                        <div class="page-title-wrap text-center w-100">
                            <div class="page-title-inner d-inline-block">
                                <h1 class="mb-0">Best Head Injury Treatment in India</h1>
                                <ol class="breadcrumb mb-0 justify-content-center">
                                    <li class="breadcrumb-item"><a href="index.php" title="Home">Home</a></li>
                                    <li class="breadcrumb-item active">Head Injury Treatment</li>
                                </ol>
                            </div>
                        </div><!-- Page Title Wrap -->
                    </div>
                </div>
            </section>
            <section>
                <div class="w-100 pt-90 pb-90 position-relative">
                    <div class="container">
                        <div class="page-wrap position-relative w-100">
                            <div class="row mrg30">
                                <div class="col-md-12 col-sm-12 col-lg-8">
                                    <div class="post-detail w-100">
                                        <div class="post-feat-img serv-detail-img brd-rd10 position-relative overflow-hidden w-100">
                                            <img class="img-fluid w-100" src="assets/img/services/Brain surgery_11zon.webp" alt="Best Head Injury Treatment in India - Dr. Arun Saroha, Neurosurgeon at Max Hospital">
                                        </div>

                                        <!-- HERO SECTION -->
                                        <h2 class="mb-0">Expert Head Injury Treatment by Dr. Arun Saroha</h2>
                                        <p class="mb-0">Head injuries are among the most life-threatening medical emergencies, and receiving the <strong>best head injury treatment in India</strong> can mean the difference between full recovery and permanent disability. Dr. Arun Saroha, Chief Neurosurgeon at Max Hospital, Delhi NCR, is one of India's most trusted experts in treating all types of traumatic brain injuries (TBI) &mdash; from mild concussions to severe brain hemorrhages.</p>

                                        <p class="mb-0">With <strong>over 26 years of neurosurgical experience</strong> and more than <strong>1,000 successful head injury surgeries</strong>, Dr. Saroha combines world-class surgical skill with compassionate patient care. He utilizes the latest neuronavigation technology, intraoperative monitoring, and minimally invasive techniques to deliver the best possible outcomes for head injury patients from across India and abroad.</p>

                                        <p class="mb-0">Whether you or your loved one has suffered a head injury from a road accident, fall, sports injury, or assault, Dr. Arun Saroha and his team provide <strong>24/7 emergency neurosurgical care</strong> with a dedicated trauma response team ready to act within the critical golden hour.</p>

                                        <!-- WHAT IS HEAD INJURY SECTION -->
                                        <h2 class="mb-0" style="margin-top:25px;">What is a Head Injury?</h2>
                                        <p class="mb-0">A head injury &mdash; medically termed a <strong>traumatic brain injury (TBI)</strong> &mdash; is any injury to the brain, skull, or scalp caused by an external mechanical force. This can include a direct blow to the head, a violent jolt or shaking, or a penetrating wound from a sharp object or projectile.</p>

                                        <p class="mb-0">Head injuries are broadly classified into two types:</p>
                                        <ul class="mb-0">
                                            <li><strong>Closed Head Injuries:</strong> The skull remains intact, but the brain is damaged by the force of impact. This includes concussions, contusions (brain bruises), and diffuse axonal injuries where nerve fibers are torn.</li>
                                            <li><strong>Open (Penetrating) Head Injuries:</strong> The skull is fractured or pierced, exposing the brain to potential infection and direct tissue damage. These often result from accidents, gunshot wounds, or falling objects.</li>
                                        </ul>

                                        <h3 class="mb-0" style="margin-top:20px;">Common Causes of Head Injuries in India</h3>
                                        <ul class="mb-0">
                                            <li><strong>Road Traffic Accidents (RTAs):</strong> The leading cause, accounting for nearly 60% of all TBI cases in India</li>
                                            <li><strong>Falls:</strong> Especially common in elderly patients and young children</li>
                                            <li><strong>Sports Injuries:</strong> Cricket, football, boxing, and cycling-related head trauma</li>
                                            <li><strong>Workplace Accidents:</strong> Construction site falls and industrial injuries</li>
                                            <li><strong>Assault and Violence:</strong> Physical attacks resulting in head trauma</li>
                                            <li><strong>Blast Injuries:</strong> Rare but severe cases from explosions</li>
                                        </ul>

                                        <h3 class="mb-0" style="margin-top:20px;">Symptoms of Head Injury &mdash; When to Seek Emergency Care</h3>
                                        <p class="mb-0">Recognizing head injury symptoms early is critical. Seek immediate medical attention if you notice any of the following after a head trauma:</p>

                                        <h4 style="margin-top:15px;margin-bottom:5px;">Mild Head Injury Symptoms:</h4>
                                        <ul class="mb-0">
                                            <li>Headache or pressure in the head</li>
                                            <li>Brief loss of consciousness (seconds to minutes)</li>
                                            <li>Dizziness, nausea, or vomiting</li>
                                            <li>Difficulty concentrating or remembering</li>
                                            <li>Ringing in the ears (tinnitus)</li>
                                            <li>Sensitivity to light or noise</li>
                                        </ul>

                                        <h4 style="margin-top:15px;margin-bottom:5px;">Severe Head Injury Symptoms (Call Emergency Immediately):</h4>
                                        <ul class="mb-0">
                                            <li>Prolonged loss of consciousness</li>
                                            <li>Repeated vomiting or persistent nausea</li>
                                            <li>Seizures or convulsions</li>
                                            <li>Unequal pupil size (one pupil larger than the other)</li>
                                            <li>Clear fluid leaking from the nose or ears (CSF leak)</li>
                                            <li>Weakness or numbness in arms or legs</li>
                                            <li>Slurred speech or inability to speak</li>
                                            <li>Increasing confusion, agitation, or unusual behavior</li>
                                            <li>Worsening headache that does not go away</li>
                                        </ul>

                                        <blockquote class="blockquote text-center thm-bg brd-rd10" style="margin-top:20px;">
                                            <i class="flaticon-quote d-block"></i>
                                            <p class="mb-0"><strong>IMPORTANT: </strong> The first 60 minutes after a severe head injury is called the "Golden Hour." Getting the patient to a neurosurgeon within this time significantly improves survival and recovery chances. Call Dr. Arun Saroha's emergency line at <strong>+91-7860000705</strong> for immediate assistance.</p>
                                        </blockquote>

                                        <!-- DIAGNOSIS SECTION -->
                                        <h2 class="mb-0" style="margin-top:25px;">How is a Head Injury Diagnosed?</h2>
                                        <p class="mb-0">Accurate and rapid diagnosis is critical in head injury management. Dr. Arun Saroha uses a multi-step diagnostic approach to determine the type, location, and severity of the injury:</p>

                                        <h3 class="mb-0" style="margin-top:20px;">Glasgow Coma Scale (GCS) Assessment</h3>
                                        <p class="mb-0">The <strong>Glasgow Coma Scale</strong> is the internationally recognized standard for assessing the severity of a head injury. It evaluates three responses:</p>
                                        <div class="table-responsive" style="margin-top:15px;margin-bottom:15px;">
                                            <table class="table table-bordered table-striped">
                                                <thead style="background-color:#2f94e1;color:#fff;">
                                                    <tr>
                                                        <th>Response</th>
                                                        <th>Score Range</th>
                                                        <th>What is Tested</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    <tr>
                                                        <td><strong>Eye Opening</strong></td>
                                                        <td>1&ndash;4</td>
                                                        <td>Spontaneous, to voice, to pain, or none</td>
                                                    </tr>
                                                    <tr>
                                                        <td><strong>Verbal Response</strong></td>
                                                        <td>1&ndash;5</td>
                                                        <td>Oriented, confused, inappropriate words, incomprehensible, none</td>
                                                    </tr>
                                                    <tr>
                                                        <td><strong>Motor Response</strong></td>
                                                        <td>1&ndash;6</td>
                                                        <td>Obeys commands, localizes pain, withdrawal, abnormal flexion, extension, none</td>
                                                    </tr>
                                                </tbody>
                                            </table>
                                        </div>
                                        <ul class="mb-0">
                                            <li><strong>Mild TBI (GCS 13&ndash;15):</strong> Patient is conscious and alert, may have brief confusion or loss of consciousness</li>
                                            <li><strong>Moderate TBI (GCS 9&ndash;12):</strong> Patient has impaired consciousness, may require close monitoring and possible surgery</li>
                                            <li><strong>Severe TBI (GCS 3&ndash;8):</strong> Patient is in a comatose state, requires emergency neurosurgical intervention and ICU care</li>
                                        </ul>

                                        <h3 class="mb-0" style="margin-top:20px;">Imaging &amp; Diagnostic Tests</h3>
                                        <ul class="mb-0">
                                            <li><strong>CT Scan (Computed Tomography):</strong> The first-line imaging tool in emergency settings. A CT scan can detect skull fractures, brain bleeding (hemorrhage), blood clots (hematomas), brain swelling, and bone fragments within minutes.</li>
                                            <li><strong>MRI (Magnetic Resonance Imaging):</strong> Provides detailed images of brain tissue and is used for detecting diffuse axonal injury, small contusions, and subtle damage not visible on CT scans. Typically used once the patient is stabilized.</li>
                                            <li><strong>X-Ray:</strong> Used to identify skull fractures, particularly linear and depressed fractures.</li>
                                            <li><strong>Cerebral Angiography:</strong> Used when vascular damage is suspected, to visualize blood vessel injuries in the brain.</li>
                                            <li><strong>Intracranial Pressure (ICP) Monitoring:</strong> A sensor is placed inside the skull to continuously measure pressure buildup, which is critical for managing severe TBI cases in the ICU.</li>
                                        </ul>

                                        <!-- TYPES OF HEAD INJURIES -->
                                        <h2 class="mb-0" style="margin-top:25px;">Types of Head Injuries We Treat</h2>
                                        <p class="mb-0">Dr. Arun Saroha is experienced in diagnosing and treating the full spectrum of head injuries:</p>

                                        <ul class="mb-0">
                                            <li><strong>Concussion:</strong> A mild TBI causing temporary brain function disruption. Most patients recover fully with proper rest and monitoring.</li>
                                            <li><strong>Brain Contusion:</strong> A bruise on the brain tissue, often caused by a direct impact. May require surgical intervention if the contusion is large or causes significant swelling.</li>
                                            <li><strong>Epidural Hematoma (EDH):</strong> Bleeding between the skull and the outer brain membrane. This is a neurosurgical emergency requiring immediate evacuation of the blood clot.</li>
                                            <li><strong>Subdural Hematoma (SDH):</strong> Bleeding between the brain surface and its covering (dura mater). Can be acute (emergency) or chronic (developing over weeks).</li>
                                            <li><strong>Subarachnoid Hemorrhage:</strong> Bleeding in the space between the brain and the thin tissues covering it. Causes severe headache and requires urgent treatment.</li>
                                            <li><strong>Diffuse Axonal Injury (DAI):</strong> Widespread damage to the brain's white matter caused by rotational acceleration. One of the most devastating forms of TBI.</li>
                                            <li><strong>Skull Fractures:</strong> Linear, depressed, or basilar fractures of the skull bones. Depressed fractures often require surgical elevation.</li>
                                            <li><strong>Penetrating Brain Injuries:</strong> Foreign objects entering the brain through the skull. Require emergency surgical exploration and repair.</li>
                                        </ul>

                                        <!-- TREATMENT OPTIONS -->
                                        <h2 class="mb-0" style="margin-top:25px;">Head Injury Treatment Options</h2>
                                        <p class="mb-0">Treatment depends on the type and severity of the head injury. Dr. Saroha employs a comprehensive approach:</p>

                                        <h3 class="mb-0" style="margin-top:20px;">Non-Surgical Treatment (Mild to Moderate Injuries)</h3>
                                        <ul class="mb-0">
                                            <li><strong>Close Observation & Monitoring:</strong> Continuous neurological assessment in a dedicated ICU setting with round-the-clock monitoring</li>
                                            <li><strong>Medications:</strong> Anti-seizure drugs, osmotic diuretics (Mannitol) to reduce brain swelling, pain management, and medications to prevent secondary brain damage</li>
                                            <li><strong>CT Scan & MRI Monitoring:</strong> Serial imaging to track brain swelling, bleeding progression, or new complications</li>
                                            <li><strong>ICP Monitoring:</strong> Intracranial pressure monitoring to detect dangerous pressure buildup inside the skull</li>
                                        </ul>

                                        <h3 class="mb-0" style="margin-top:20px;">Surgical Treatment (Moderate to Severe Injuries)</h3>
                                        <ul class="mb-0">
                                            <li><strong>Craniotomy:</strong> Surgical opening of the skull to remove blood clots (hematomas), repair damaged blood vessels, or remove bone fragments</li>
                                            <li><strong>Decompressive Craniectomy:</strong> Removal of a portion of the skull to allow the swollen brain to expand, reducing life-threatening pressure. The bone flap is preserved and replaced later.</li>
                                            <li><strong>Hematoma Evacuation:</strong> Emergency removal of epidural or subdural blood clots that are compressing the brain</li>
                                            <li><strong>Depressed Fracture Elevation:</strong> Surgical lifting and repair of depressed skull fractures that are pressing on the brain</li>
                                            <li><strong>CSF Drainage:</strong> Placement of external ventricular drain (EVD) to remove excess cerebrospinal fluid and reduce intracranial pressure</li>
                                            <li><strong>Cranioplasty:</strong> Reconstruction of the skull using the patient's own bone or synthetic materials after decompressive surgery</li>
                                        </ul>

                                        <!-- TREATMENT COST -->
                                        <h2 class="mb-0" style="margin-top:25px;">Head Injury Treatment Cost in India</h2>
                                        <p class="mb-0">India offers world-class head injury treatment at a fraction of the cost compared to Western countries. Here is an approximate cost guide:</p>

                                        <div class="table-responsive" style="margin-top:15px;margin-bottom:15px;">
                                            <table class="table table-bordered table-striped">
                                                <thead style="background-color:#2f94e1;color:#fff;">
                                                    <tr>
                                                        <th>Treatment Type</th>
                                                        <th>Approximate Cost (INR)</th>
                                                        <th>Approximate Cost (USD)</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    <tr>
                                                        <td>Mild Head Injury (Observation &amp; Monitoring)</td>
                                                        <td>&#8377;50,000 &ndash; &#8377;1,50,000</td>
                                                        <td>$600 &ndash; $1,800</td>
                                                    </tr>
                                                    <tr>
                                                        <td>CT Scan / MRI Brain</td>
                                                        <td>&#8377;3,000 &ndash; &#8377;15,000</td>
                                                        <td>$35 &ndash; $180</td>
                                                    </tr>
                                                    <tr>
                                                        <td>Craniotomy / Hematoma Evacuation</td>
                                                        <td>&#8377;2,00,000 &ndash; &#8377;5,00,000</td>
                                                        <td>$2,400 &ndash; $6,000</td>
                                                    </tr>
                                                    <tr>
                                                        <td>Decompressive Craniectomy</td>
                                                        <td>&#8377;3,00,000 &ndash; &#8377;6,00,000</td>
                                                        <td>$3,600 &ndash; $7,200</td>
                                                    </tr>
                                                    <tr>
                                                        <td>ICU Stay (Per Day)</td>
                                                        <td>&#8377;15,000 &ndash; &#8377;35,000</td>
                                                        <td>$180 &ndash; $420</td>
                                                    </tr>
                                                    <tr>
                                                        <td>Cranioplasty (Skull Reconstruction)</td>
                                                        <td>&#8377;2,50,000 &ndash; &#8377;5,00,000</td>
                                                        <td>$3,000 &ndash; $6,000</td>
                                                    </tr>
                                                    <tr>
                                                        <td>Neuro Rehabilitation (Per Month)</td>
                                                        <td>&#8377;50,000 &ndash; &#8377;1,50,000</td>
                                                        <td>$600 &ndash; $1,800</td>
                                                    </tr>
                                                </tbody>
                                            </table>
                                        </div>
                                        <p class="mb-0"><em>Note: Costs may vary based on the severity of injury, hospital stay duration, and specific treatment requirements. Contact us at <strong>+91-7860000705</strong> for a personalized cost estimate.</em></p>

                                        <!-- RECOVERY TIMELINE -->
                                        <h2 class="mb-0" style="margin-top:25px;">Recovery Timeline After Head Injury Treatment</h2>
                                        <p class="mb-0">Recovery from a head injury depends on the severity, the patient's age and overall health, and the quality of rehabilitation received:</p>

                                        <div class="table-responsive" style="margin-top:15px;margin-bottom:15px;">
                                            <table class="table table-bordered table-striped">
                                                <thead style="background-color:#2f94e1;color:#fff;">
                                                    <tr>
                                                        <th>Injury Severity</th>
                                                        <th>Hospital Stay</th>
                                                        <th>Initial Recovery</th>
                                                        <th>Full Recovery</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    <tr>
                                                        <td><strong>Mild (Concussion)</strong></td>
                                                        <td>24-48 hours observation</td>
                                                        <td>1-2 weeks</td>
                                                        <td>2-4 weeks</td>
                                                    </tr>
                                                    <tr>
                                                        <td><strong>Moderate</strong></td>
                                                        <td>1-3 weeks</td>
                                                        <td>1-3 months</td>
                                                        <td>3-6 months</td>
                                                    </tr>
                                                    <tr>
                                                        <td><strong>Severe</strong></td>
                                                        <td>3-8 weeks (including ICU)</td>
                                                        <td>3-6 months</td>
                                                        <td>6-18 months+</td>
                                                    </tr>
                                                    <tr>
                                                        <td><strong>Very Severe (DAI/Multiple Injuries)</strong></td>
                                                        <td>4-12 weeks</td>
                                                        <td>6-12 months</td>
                                                        <td>12-24 months+ (ongoing rehab)</td>
                                                    </tr>
                                                </tbody>
                                            </table>
                                        </div>

                                        <p class="mb-0">Dr. Saroha's team provides comprehensive post-surgical rehabilitation support including physiotherapy, speech therapy, occupational therapy, and neuropsychological counseling to ensure the best possible recovery outcomes.</p>

                                        <!-- WHY CHOOSE DR ARUN SAROHA -->
                                        <div class="team-detail-wrap" style="margin-top:25px;">
                                            <h2 class="mb-0">Why Choose Dr. Arun Saroha for Head Injury Treatment in India?</h2>
                                            <p class="mb-0">Dr. Arun Saroha is widely recognized as one of the <strong>best neurosurgeons in India for head injury treatment</strong>. Here is what sets him apart:</p>
                                            <ul class="mb-0 list-unstyled">
                                                <li><strong>26+ Years of Neurosurgical Expertise:</strong> Two decades of experience in treating the most complex head injury cases, from emergency trauma to delayed complications.</li>
                                                <li><strong>1,000+ Successful Brain Surgeries:</strong> A proven track record of successful outcomes in craniotomies, hematoma evacuations, decompressive craniectomies, and skull reconstruction surgeries.</li>
                                                <li><strong>Advanced Surgical Technology:</strong> Access to state-of-the-art neuronavigation systems, intraoperative CT scanning, microsurgical instruments, and advanced ICU monitoring at Max Hospital.</li>
                                                <li><strong>24/7 Emergency Availability:</strong> Dr. Saroha and his trauma team are available round-the-clock for emergency head injury cases, ensuring treatment within the critical golden hour.</li>
                                                <li><strong>Max Hospital Infrastructure:</strong> Operating at Max Super Speciality Hospital (Gurgaon and Dwarka, Delhi) &mdash; a NABH and JCI accredited facility with dedicated neuro ICU, advanced imaging, and fully equipped operation theaters.</li>
                                                <li><strong>Comprehensive Rehabilitation:</strong> In-house physiotherapy, speech therapy, and neurological rehabilitation to support complete recovery after head injury surgery.</li>
                                                <li><strong>International Patient Care:</strong> Dedicated international patient desk providing visa assistance, airport pickup, accommodation support, and interpreter services for patients traveling from abroad.</li>
                                                <li><strong>Compassionate Patient-First Approach:</strong> Dr. Saroha is known for taking the time to explain every aspect of treatment to patients and families, ensuring they are informed and comfortable throughout the journey.</li>
                                            </ul>

                                        </div>

                                        <!-- PREVENTION SECTION -->
                                        <h3 class="mb-0" style="margin-top:25px;">How to Prevent Head Injuries</h3>
                                        <ul class="mb-0">
                                            <li>Always wear a helmet while riding two-wheelers, cycling, or participating in contact sports</li>
                                            <li>Use seatbelts in all vehicles and ensure children are in appropriate car seats</li>
                                            <li>Install safety gates and window guards at home for young children</li>
                                            <li>Use non-slip mats in bathrooms and ensure adequate lighting on stairs for elderly family members</li>
                                            <li>Follow workplace safety protocols and use protective headgear at construction sites</li>
                                            <li>Avoid driving under the influence of alcohol or drugs</li>
                                            <li>Supervise children during play and sports activities</li>
                                        </ul>

                                        <!-- HEAD INJURY IN CHILDREN -->
                                        <h2 class="mb-0" style="margin-top:25px;">Head Injury in Children &mdash; Special Considerations</h2>
                                        <p class="mb-0">Children are especially vulnerable to head injuries due to their larger head-to-body ratio, thinner skull bones, and still-developing brains. Pediatric head injuries require specialized neurosurgical expertise, and Dr. Arun Saroha has extensive experience treating children of all ages.</p>

                                        <h4 style="margin-top:15px;margin-bottom:5px;">Common Causes in Children:</h4>
                                        <ul class="mb-0">
                                            <li>Falls from beds, stairs, playground equipment, or while learning to walk</li>
                                            <li>Sports-related injuries (cricket, football, cycling)</li>
                                            <li>Road traffic accidents (especially as pedestrians or bicycle riders)</li>
                                            <li>Accidental hits from falling objects</li>
                                        </ul>

                                        <h4 style="margin-top:15px;margin-bottom:5px;">Warning Signs in Children (Seek Immediate Help):</h4>
                                        <ul class="mb-0">
                                            <li>Persistent crying that cannot be consoled</li>
                                            <li>Refusal to eat or breastfeed</li>
                                            <li>Vomiting repeatedly after head trauma</li>
                                            <li>Unusual drowsiness or difficulty waking up</li>
                                            <li>Bulging fontanelle (soft spot) in infants</li>
                                            <li>Seizures, unsteady walking, or loss of previously learned skills</li>
                                            <li>Any loss of consciousness, even brief</li>
                                        </ul>

                                        <p class="mb-0" style="margin-top:15px;">At Max Hospital, Dr. Saroha's team is equipped with pediatric neuro ICU facilities and child-friendly rehabilitation services to ensure the best possible outcomes for young patients. <strong>If your child has suffered a head injury, call +91-7860000705 for immediate consultation.</strong></p>

                                        <!-- FIRST AID SECTION -->
                                        <h2 class="mb-0" style="margin-top:25px;">First Aid for Head Injuries &mdash; What to Do Before Reaching the Hospital</h2>
                                        <p class="mb-0">Knowing what to do immediately after a head injury can save a life. Follow these steps while waiting for emergency help:</p>
                                        <ul class="mb-0">
                                            <li><strong>Call emergency services immediately</strong> if the person is unconscious, bleeding heavily, or showing signs of severe injury</li>
                                            <li><strong>Do not move the person</strong> if you suspect a spinal injury &mdash; keep the head and neck still</li>
                                            <li><strong>Apply gentle pressure</strong> with a clean cloth to stop any bleeding from the scalp</li>
                                            <li><strong>Do not remove objects</strong> embedded in the skull &mdash; stabilize them and wait for medical help</li>
                                            <li><strong>Monitor breathing and consciousness</strong> &mdash; if the person stops breathing, begin CPR if trained</li>
                                            <li><strong>Keep the person still and calm</strong> &mdash; do not give food, water, or medication</li>
                                            <li><strong>Note the time</strong> of the injury and any symptoms for the medical team</li>
                                        </ul>

                                    </div>
                                </div>
                                <div class="col-md-6 col-sm-8 col-lg-4">
                                    <?php include 'service-sidebar.php';?>
                                </div>
                            </div>
                        </div><!-- Page Wrap -->
                    </div>
                </div>
            </section>


            <section>
                <div class="container">
                    <div class=" position-relative w-100">
                                        <div class="accordion">
                                            <div style="border-bottom: 3px solid #2f94e1;">
                                                <h2 class="mb-0 text-color3">Frequently Asked Questions About Head Injury Treatment</h2>
                                            </div>

                                        <!--question 1-->
                                          <div class="accordion__item">
                                            <button class="accordion__btn">

                                              <span class="accordion__caption"><i class="far fa-lightbulb"></i>What is a head injury and how serious can it be?</span>
                                              <span class="accordion__icon"><i class="fa fa-plus"></i></span>
                                            </button>

                                            <div class="accordion__content">
                                              <p>A head injury, also known as traumatic brain injury (TBI), is any injury to the brain, skull, or scalp caused by external force such as a blow, jolt, or penetrating wound. Head injuries range from mild concussions that resolve in days to severe brain damage that can be life-threatening or cause permanent disability. Early diagnosis and treatment by an experienced neurosurgeon significantly improves outcomes.</p>
                                            </div>
                                          </div>
                                        <!--question 2-->
                                          <div class="accordion__item">
                                            <button class="accordion__btn">

                                              <span class="accordion__caption"><i class="far fa-lightbulb"></i>What are the warning signs of a serious head injury?</span>
                                              <span class="accordion__icon"><i class="fa fa-plus"></i></span>
                                            </button>

                                            <div class="accordion__content">
                                              <p>Warning signs of a serious head injury include: prolonged loss of consciousness, repeated vomiting, seizures, unequal pupil size, clear fluid from nose or ears, severe headache that worsens, weakness or numbness in limbs, slurred speech, increasing confusion, and difficulty waking from sleep. If any of these symptoms appear after a head trauma, seek emergency neurosurgical care immediately by calling +91-7860000705.</p>
                                            </div>
                                          </div>
                                        <!--question 3-->
                                          <div class="accordion__item">
                                            <button class="accordion__btn">

                                              <span class="accordion__caption"><i class="far fa-lightbulb"></i>What causes head injuries?</span>
                                              <span class="accordion__icon"><i class="fa fa-plus"></i></span>
                                            </button>

                                            <div class="accordion__content">
                                              <p>The most common causes of head injuries in India include road traffic accidents (the leading cause at nearly 60%), falls (especially in elderly and children), sports-related injuries, workplace accidents, physical assault, and falling objects. Two-wheeler accidents without helmets are a particularly significant cause of severe TBI in India.</p>
                                            </div>
                                          </div>
                                        <!--question 4-->
                                          <div class="accordion__item">
                                            <button class="accordion__btn">

                                              <span class="accordion__caption"><i class="far fa-lightbulb"></i>How is a head injury diagnosed?</span>
                                              <span class="accordion__icon"><i class="fa fa-plus"></i></span>
                                            </button>

                                            <div class="accordion__content">
                                              <p>Head injury diagnosis involves a thorough neurological examination using the Glasgow Coma Scale (GCS), followed by CT scan (the primary imaging tool for emergency cases) and MRI for detailed brain tissue assessment. Additional tests may include X-rays for skull fractures, angiography for blood vessel damage, and intracranial pressure (ICP) monitoring for severe cases.</p>
                                            </div>
                                          </div>

                                          <!--question 5-->
                                          <div class="accordion__item">
                                            <button class="accordion__btn">

                                              <span class="accordion__caption"><i class="far fa-lightbulb"></i>What are the treatment options for head injuries?</span>
                                              <span class="accordion__icon"><i class="fa fa-plus"></i></span>
                                            </button>

                                            <div class="accordion__content">
                                              <p>Treatment depends on severity. Mild injuries (concussions) are managed with rest, observation, and pain medication. Moderate to severe injuries may require surgical intervention including craniotomy (opening the skull to remove blood clots), decompressive craniectomy (removing skull bone to relieve pressure), hematoma evacuation, depressed fracture repair, and ICP monitoring. Dr. Arun Saroha determines the best approach based on each patient's specific condition.</p>
                                            </div>
                                          </div>

                                          <!--question 6-->
                                          <div class="accordion__item">
                                            <button class="accordion__btn">

                                              <span class="accordion__caption"><i class="far fa-lightbulb"></i>How much does head injury treatment cost in India?</span>
                                              <span class="accordion__icon"><i class="fa fa-plus"></i></span>
                                            </button>

                                            <div class="accordion__content">
                                              <p>Head injury treatment costs in India vary based on severity. Mild injury observation costs INR 50,000-1,50,000. Surgical procedures like craniotomy or hematoma evacuation range from INR 2,00,000-5,00,000. Decompressive craniectomy costs INR 3,00,000-6,00,000. ICU stays cost INR 15,000-35,000 per day. These costs are significantly lower than Western countries while maintaining world-class care standards. Contact us at +91-7860000705 for a personalized estimate.</p>
                                            </div>
                                          </div>

                                          <!--question 7-->
                                          <div class="accordion__item">
                                            <button class="accordion__btn">

                                              <span class="accordion__caption"><i class="far fa-lightbulb"></i>What is the recovery time after head injury surgery?</span>
                                              <span class="accordion__icon"><i class="fa fa-plus"></i></span>
                                            </button>

                                            <div class="accordion__content">
                                              <p>Recovery time varies significantly based on injury severity. Mild concussions typically resolve in 2-4 weeks. Moderate head injuries require 3-6 months for recovery. Severe TBI cases may need 6-18 months or more of rehabilitation. Factors affecting recovery include patient age, overall health, extent of brain damage, and quality of post-surgical rehabilitation. Dr. Saroha's team provides comprehensive rehabilitation support throughout the recovery journey.</p>
                                            </div>
                                          </div>

                                          <!--question 8-->
                                          <div class="accordion__item">
                                            <button class="accordion__btn">

                                              <span class="accordion__caption"><i class="far fa-lightbulb"></i>What is the Golden Hour in head injury treatment?</span>
                                              <span class="accordion__icon"><i class="fa fa-plus"></i></span>
                                            </button>

                                            <div class="accordion__content">
                                              <p>The Golden Hour refers to the first 60 minutes after a severe head injury. Getting the patient to a qualified neurosurgeon within this critical window dramatically improves survival rates and recovery outcomes. During this time, life-saving interventions like stopping brain bleeding, reducing intracranial pressure, and preventing secondary brain damage can be performed. Dr. Arun Saroha's team at Max Hospital is available 24/7 for emergency head injury cases.</p>
                                            </div>
                                          </div>

                                          <!--question 9-->
                                          <div class="accordion__item">
                                            <button class="accordion__btn">

                                              <span class="accordion__caption"><i class="far fa-lightbulb"></i>Can a head injury cause long-term complications?</span>
                                              <span class="accordion__icon"><i class="fa fa-plus"></i></span>
                                            </button>

                                            <div class="accordion__content">
                                              <p>Yes, depending on severity. Possible long-term complications include post-traumatic epilepsy (seizures), cognitive difficulties (memory, concentration), behavioral and personality changes, chronic headaches, balance problems, speech difficulties, and in severe cases, physical disability. However, with expert treatment from a skilled neurosurgeon like Dr. Saroha, followed by proper rehabilitation, many patients achieve significant recovery and return to their daily activities.</p>
                                            </div>
                                          </div>

                                          <!--question 10-->
                                          <div class="accordion__item">
                                            <button class="accordion__btn">

                                              <span class="accordion__caption"><i class="far fa-lightbulb"></i>Is head injury treatment available for international patients?</span>
                                              <span class="accordion__icon"><i class="fa fa-plus"></i></span>
                                            </button>

                                            <div class="accordion__content">
                                              <p>Yes, Dr. Arun Saroha and Spine and Brain India regularly treat international patients from the Middle East, Africa, and Southeast Asia. We provide complete assistance including medical visa support, airport pickup, accommodation arrangements, interpreter services, and telemedicine follow-up consultations. India offers the best head injury treatment at 60-80% lower costs compared to the US, UK, or Europe, making it a preferred destination for medical tourism.</p>
                                            </div>
                                          </div>

                                          <!--question 11-->
                                          <div class="accordion__item">
                                            <button class="accordion__btn">

                                              <span class="accordion__caption"><i class="far fa-lightbulb"></i>What rehabilitation is needed after head injury surgery?</span>
                                              <span class="accordion__icon"><i class="fa fa-plus"></i></span>
                                            </button>

                                            <div class="accordion__content">
                                              <p>Post-surgical rehabilitation is crucial for optimal recovery and may include: physiotherapy (for motor function and strength), occupational therapy (for daily living skills), speech and language therapy, cognitive rehabilitation (for memory and thinking skills), neuropsychological counseling, and nutritional support. Dr. Saroha's team creates personalized rehabilitation plans for each patient based on their specific deficits and recovery goals.</p>
                                            </div>
                                          </div>

                                          <!--question 12-->
                                          <div class="accordion__item">
                                            <button class="accordion__btn">

                                              <span class="accordion__caption"><i class="far fa-lightbulb"></i>How can head injuries be prevented?</span>
                                              <span class="accordion__icon"><i class="fa fa-plus"></i></span>
                                            </button>

                                            <div class="accordion__content">
                                              <p>Key prevention measures include: always wearing helmets while riding two-wheelers or cycling, using seatbelts in vehicles, installing safety gates for children, using non-slip mats for elderly, following workplace safety protocols, avoiding driving under the influence of alcohol, supervising children during sports, and wearing protective gear during contact sports. While not all head injuries can be prevented, these precautions significantly reduce the risk.</p>
                                            </div>
                                          </div>

                                        </div>
                                    </div>
                </div>
            </section>

                   <section>
                <div class="w-100 scndry-layer opc98 position-relative">
                    <div class="fixed-bg patern-bg" style="background-image: url(assets/images/pattern-bg1.jpg);"></div>
                    <div class="testi-storie-wrap w-100">
                        <div class="row mrg">
                            <div class="col-md-12 col-sm-12 col-lg-6">
                                <div class="testi-wrap d-flex flex-wrap justify-content-center align-items-center position-relative w-100">
                                    <div class="testi-nav-caro">
                                        <div class="testi-nav-item rounded-circle"><img class="img-fluid rounded-circle" src="assets/img/testi.jpg" alt="Head Injury Patient Testimonial"></div>
                                        <div class="testi-nav-item rounded-circle"><img class="img-fluid rounded-circle" src="assets/img/testi.jpg" alt="Brain Surgery Patient Review"></div>
                                        <div class="testi-nav-item rounded-circle"><img class="img-fluid rounded-circle" src="assets/img/testi.jpg" alt="TBI Treatment Patient Testimonial"></div>
                                        <div class="testi-nav-item rounded-circle"><img class="img-fluid rounded-circle" src="assets/img/testi.jpg" alt="Neurosurgery Patient Testimonial"></div>
                                    </div>
                                    <div class="testi-inner d-inline-block w-100">
                                        <div class="testi-list-caro">
                                            <div class="testi-item-wrap d-block w-100">
                                                <div class="testi-item-box position-relative brd-rd10 w-100">
                                                    <div class="testi-img rounded-circle position-absolute">
                                                        <img class="img-fluid rounded-circle" src="assets/img/testi.jpg" alt="Priya Sharma - Head Injury Patient">
                                                        <i class="flaticon-left-quotes-sign rounded-circle text-center scndry-clr"></i></div>
                                                    <div class="testi-item-top d-flex flex-wrap align-items-center justify-content-between w-100">
                                                        <div class="testi-item-info">
                                                            <h4 class="mb-0">Priya Sharma</h4>
                                                            <span class="d-block thm-clr">Road Accident - Subdural Hematoma</span>
                                                        </div>
                                                        <span class="rate d-inline-block thm-clr"><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i></span>
                                                    </div>
                                                    <p class="mb-0">Dr. Arun Saroha saved my life after a severe car accident. I had a subdural hematoma and was rushed to Max Hospital where Dr. Saroha performed emergency surgery within the golden hour. After 3 months of recovery and rehabilitation, I am now back to my normal life. His expertise in head injury treatment is truly unmatched. I am forever grateful.</p>
                                                </div>
                                            </div>
                                            <div class="testi-item-wrap d-block w-100">
                                                <div class="testi-item-box position-relative brd-rd10 w-100">
                                                    <div class="testi-img rounded-circle position-absolute">
                                                        <img class="img-fluid rounded-circle" src="assets/img/testi.jpg" alt="Rajesh Patel - Brain Injury Patient">
                                                        <i class="flaticon-left-quotes-sign rounded-circle text-center scndry-clr"></i></div>
                                                    <div class="testi-item-top d-flex flex-wrap align-items-center justify-content-between w-100">
                                                        <div class="testi-item-info">
                                                            <h4 class="mb-0">Rajesh Patel</h4>
                                                            <span class="d-block thm-clr">Fall Injury - Epidural Hematoma</span>
                                                        </div>
                                                        <span class="rate d-inline-block thm-clr"><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i></span>
                                                    </div>
                                                    <p class="mb-0">My father fell from a ladder and suffered an epidural hematoma. Dr. Saroha's team responded within minutes and performed a craniotomy that saved his life. The post-operative care was excellent, and the rehabilitation team helped him regain full mobility. The entire family is thankful to Dr. Saroha for his exceptional skill and compassion.</p>
                                                </div>
                                            </div>
                                            <div class="testi-item-wrap d-block w-100">
                                                <div class="testi-item-box position-relative brd-rd10 w-100">
                                                    <div class="testi-img rounded-circle position-absolute">
                                                        <img class="img-fluid rounded-circle" src="assets/img/testi.jpg" alt="Meera Bhaskar - TBI Patient">
                                                        <i class="flaticon-left-quotes-sign rounded-circle text-center scndry-clr"></i></div>
                                                    <div class="testi-item-top d-flex flex-wrap align-items-center justify-content-between w-100">
                                                        <div class="testi-item-info">
                                                            <h4 class="mb-0">Meera Bhaskar</h4>
                                                            <span class="d-block thm-clr">Sports Injury - Concussion & Skull Fracture</span>
                                                        </div>
                                                        <span class="rate d-inline-block thm-clr"><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i></span>
                                                    </div>
                                                    <p class="mb-0">My son had a depressed skull fracture from a cricket injury. We were terrified, but Dr. Saroha explained everything clearly and operated with incredible precision. The surgery was successful, and my son is now completely recovered and back to playing sports. Dr. Saroha is the best neurosurgeon for head injuries in India, without a doubt.</p>
                                                </div>
                                            </div>
                                            <div class="testi-item-wrap d-block w-100">
                                                <div class="testi-item-box position-relative brd-rd10 w-100">
                                                    <div class="testi-img rounded-circle position-absolute">
                                                        <img class="img-fluid rounded-circle" src="assets/img/testi.jpg" alt="Ravi Gupta - Brain Surgery Patient">
                                                        <i class="flaticon-left-quotes-sign rounded-circle text-center scndry-clr"></i></div>
                                                    <div class="testi-item-top d-flex flex-wrap align-items-center justify-content-between w-100">
                                                        <div class="testi-item-info">
                                                            <h4 class="mb-0">Ravi Gupta</h4>
                                                            <span class="d-block thm-clr">Bike Accident - Severe TBI</span>
                                                        </div>
                                                        <span class="rate d-inline-block thm-clr"><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i></span>
                                                    </div>
                                                    <p class="mb-0">I had a severe traumatic brain injury after a bike accident and was in a coma for 5 days. Dr. Saroha performed a decompressive craniectomy followed by cranioplasty months later. His round-the-clock care during those critical days made all the difference. After 8 months of rehabilitation guided by his team, I have recovered almost fully. He truly gives patients their lives back.</p>
                                                </div>
                                            </div>
                                            <div class="testi-item-wrap d-block w-100">
                                                <div class="testi-item-box position-relative brd-rd10 w-100">
                                                    <div class="testi-img rounded-circle position-absolute">
                                                        <img class="img-fluid rounded-circle" src="assets/img/testi.jpg" alt="Shivani Mehta - Brain Injury Patient">
                                                        <i class="flaticon-left-quotes-sign rounded-circle text-center scndry-clr"></i></div>
                                                    <div class="testi-item-top d-flex flex-wrap align-items-center justify-content-between w-100">
                                                        <div class="testi-item-info">
                                                            <h4 class="mb-0">Shivani Mehta</h4>
                                                            <span class="d-block thm-clr">Fall Injury - Brain Contusion</span>
                                                        </div>
                                                        <span class="rate d-inline-block thm-clr"><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i></span>
                                                    </div>
                                                    <p class="mb-0">After a bad fall at home, I was diagnosed with a brain contusion and was losing consciousness frequently. Dr. Saroha admitted me immediately, monitored the swelling carefully, and managed it without surgery through expert medical management. His decision-making saved me from unnecessary surgery while still ensuring my full recovery. Truly the best head injury doctor in India.</p>
                                                </div>
                                            </div>
                                            <div class="testi-item-wrap d-block w-100">
                                                <div class="testi-item-box position-relative brd-rd10 w-100">
                                                    <div class="testi-img rounded-circle position-absolute">
                                                        <img class="img-fluid rounded-circle" src="assets/img/testi.jpg" alt="Vikram Singh - Neurosurgery Patient">
                                                        <i class="flaticon-left-quotes-sign rounded-circle text-center scndry-clr"></i></div>
                                                    <div class="testi-item-top d-flex flex-wrap align-items-center justify-content-between w-100">
                                                        <div class="testi-item-info">
                                                            <h4 class="mb-0">Vikram Singh</h4>
                                                            <span class="d-block thm-clr">Workplace Accident - Depressed Skull Fracture</span>
                                                        </div>
                                                        <span class="rate d-inline-block thm-clr"><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i></span>
                                                    </div>
                                                    <p class="mb-0">I suffered a depressed skull fracture at a construction site. Dr. Saroha performed emergency surgery to elevate the fracture and repair the damage. His team was incredibly professional and kept my family informed throughout the process. Within 4 months I was back at work. The treatment cost was also very reasonable compared to what I expected. Highly recommended.</p>
                                                </div>
                                            </div>
                                            <div class="testi-item-wrap d-block w-100">
                                                <div class="testi-item-box position-relative brd-rd10 w-100">
                                                    <div class="testi-img rounded-circle position-absolute">
                                                        <img class="img-fluid rounded-circle" src="assets/img/testi.jpg" alt="Ahmed Al-Rashid - International Patient">
                                                        <i class="flaticon-left-quotes-sign rounded-circle text-center scndry-clr"></i></div>
                                                    <div class="testi-item-top d-flex flex-wrap align-items-center justify-content-between w-100">
                                                        <div class="testi-item-info">
                                                            <h4 class="mb-0">Ahmed Al-Rashid</h4>
                                                            <span class="d-block thm-clr">International Patient - Chronic Subdural Hematoma</span>
                                                        </div>
                                                        <span class="rate d-inline-block thm-clr"><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i></span>
                                                    </div>
                                                    <p class="mb-0">I traveled from Oman to India specifically for Dr. Saroha's expertise in treating my chronic subdural hematoma. The international patient team arranged everything from visa to accommodation. Dr. Saroha's surgery was successful and the care quality was at par with any hospital in the world, at a fraction of the cost. I recommend him to anyone seeking the best head injury treatment in India.</p>
                                                </div>
                                            </div>
                                            <div class="testi-item-wrap d-block w-100">
                                                <div class="testi-item-box position-relative brd-rd10 w-100">
                                                    <div class="testi-img rounded-circle position-absolute">
                                                        <img class="img-fluid rounded-circle" src="assets/img/testi.jpg" alt="Sunita Devi - Head Injury Patient">
                                                        <i class="flaticon-left-quotes-sign rounded-circle text-center scndry-clr"></i></div>
                                                    <div class="testi-item-top d-flex flex-wrap align-items-center justify-content-between w-100">
                                                        <div class="testi-item-info">
                                                            <h4 class="mb-0">Sunita Devi</h4>
                                                            <span class="d-block thm-clr">Road Accident - Multiple Brain Hemorrhages</span>
                                                        </div>
                                                        <span class="rate d-inline-block thm-clr"><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i></span>
                                                    </div>
                                                    <p class="mb-0">My husband was in a critical condition after a highway accident with multiple brain hemorrhages. Other hospitals refused to operate, but Dr. Saroha took the case immediately. He performed two surgeries over 3 weeks and saved my husband's life. The rehabilitation team helped him walk and talk again. We came from Bihar and the entire treatment, including hospital stay, was managed within our budget. God bless Dr. Saroha.</p>
                                                </div>
                                            </div>

                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-12 col-sm-12 col-lg-6">
                                <div class="storie-wrap text-center d-flex flex-wrap justify-content-center align-items-center overflow-hidden position-relative w-100" style="background-image: url(assets/img/testi.webp);">
                                    <div class="storie-inner d-inline-block w-100">
                                        <i class="flaticon-neurology d-inline-block"></i>
                                        <h2 class="mb-0">What Our Head Injury Patients Say</h2>
                                        <p class="mb-0">Real stories of recovery from patients treated by Dr. Arun Saroha</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div><!-- Testimonials & Stories Wrap -->
                </div>
            </section>
            <?php include 'single-line-form.php';?>


<!-- SBI YouTube Videos Section -->
<section class="sbi-vid" aria-label="Videos by Dr. Arun Saroha">
  <div class="sbi-vid-wrap">
    <div class="sbi-vid-head">
      <span class="sbi-vid-eyebrow"><svg width="18" height="18" viewBox="0 0 24 24" fill="#dc2626" aria-hidden="true"><path d="M23 12s0-3.9-.5-5.8a3 3 0 0 0-2.1-2.1C18.5 3.6 12 3.6 12 3.6s-6.5 0-8.4.5A3 3 0 0 0 1.5 6.2C1 8.1 1 12 1 12s0 3.9.5 5.8a3 3 0 0 0 2.1 2.1c1.9.5 8.4.5 8.4.5s6.5 0 8.4-.5a3 3 0 0 0 2.1-2.1C23 15.9 23 12 23 12zM9.8 15.5v-7l6.2 3.5-6.2 3.5z"/></svg> Video Library</span>
      <h2>Watch: Head Injury Videos by Dr. Arun Saroha</h2>
      <p>Dr. Arun Saroha, one of India&rsquo;s leading brain &amp; spine surgeons, shares patient testimonials and expert insights on neurological conditions and brain care.</p>
    </div>
    <div class="sbi-vid-grid">
        <div class="sbi-vid-card">
          <div class="sbi-vid-frame">
            <button type="button" class="sbi-vid-thumb" data-ytid="YaqEacpw-kE" data-title="Understanding a Complex Neurological Condition" style="background-image:url('https://i.ytimg.com/vi/YaqEacpw-kE/hqdefault.jpg')" aria-label="Play video: Understanding a Complex Neurological Condition"><span class="sbi-vid-play"></span></button>
          </div>
          <p class="sbi-vid-title">Understanding a Complex Neurological Condition</p>
        </div>
        <div class="sbi-vid-card">
          <div class="sbi-vid-frame">
            <button type="button" class="sbi-vid-thumb" data-ytid="TCogn2SY3ys" data-title="When Should You See a Neurologist?" style="background-image:url('https://i.ytimg.com/vi/TCogn2SY3ys/hqdefault.jpg')" aria-label="Play video: When Should You See a Neurologist?"><span class="sbi-vid-play"></span></button>
          </div>
          <p class="sbi-vid-title">When Should You See a Neurologist?</p>
        </div>
        <div class="sbi-vid-card">
          <div class="sbi-vid-frame">
            <button type="button" class="sbi-vid-thumb" data-ytid="ki888H99xzM" data-title="Is Coffee Good for Your Brain?" style="background-image:url('https://i.ytimg.com/vi/ki888H99xzM/hqdefault.jpg')" aria-label="Play video: Is Coffee Good for Your Brain?"><span class="sbi-vid-play"></span></button>
          </div>
          <p class="sbi-vid-title">Is Coffee Good for Your Brain?</p>
        </div>
    </div>
  </div>
</section>
<script type="application/ld+json">{"@context":"https://schema.org","@graph":[{"@type":"VideoObject","name":"Understanding a Complex Neurological Condition","description":"Dr. Arun Saroha explains the intricacies of a complex neurological condition.","thumbnailUrl":"https://i.ytimg.com/vi/YaqEacpw-kE/hqdefault.jpg","uploadDate":"2024-02-19","contentUrl":"https://www.youtube.com/watch?v=YaqEacpw-kE","embedUrl":"https://www.youtube-nocookie.com/embed/YaqEacpw-kE","publisher":{"@type":"Organization","name":"Spine and Brain India","logo":{"@type":"ImageObject","url":"https://spineandbrainindia.com/assets/img/logo/logoSSB%207777777_11zon.webp"}}},{"@type":"VideoObject","name":"When Should You See a Neurologist?","description":"Dr. Arun Saroha explains the warning signs that mean you should consult a neurologist.","thumbnailUrl":"https://i.ytimg.com/vi/TCogn2SY3ys/hqdefault.jpg","uploadDate":"2025-08-01","contentUrl":"https://www.youtube.com/watch?v=TCogn2SY3ys","embedUrl":"https://www.youtube-nocookie.com/embed/TCogn2SY3ys","publisher":{"@type":"Organization","name":"Spine and Brain India","logo":{"@type":"ImageObject","url":"https://spineandbrainindia.com/assets/img/logo/logoSSB%207777777_11zon.webp"}}},{"@type":"VideoObject","name":"Is Coffee Good for Your Brain?","description":"Dr. Arun Saroha discusses caffeine and its effects on brain health.","thumbnailUrl":"https://i.ytimg.com/vi/ki888H99xzM/hqdefault.jpg","uploadDate":"2025-08-13","contentUrl":"https://www.youtube.com/watch?v=ki888H99xzM","embedUrl":"https://www.youtube-nocookie.com/embed/ki888H99xzM","publisher":{"@type":"Organization","name":"Spine and Brain India","logo":{"@type":"ImageObject","url":"https://spineandbrainindia.com/assets/img/logo/logoSSB%207777777_11zon.webp"}}}]}</script>
<!-- End SBI YouTube Videos Section -->

<?php include 'footer.php';?>